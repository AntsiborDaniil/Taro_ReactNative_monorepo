...
function DailyTarotLimitModal() {
  ...
  const { authUser, refreshAuthSession } = useData({ Context: UserContext });
  ...
  const handleBuy = useCallback(async () => {
    ...
    if (!trimmed.includes('@')) {
      setError(t('dailyLimit.emailInvalid'));
      return;
    }
    ...
    try {
      ...
      if (!result.ok || !result.data?.paymentUrl) {
        if (result.status === 503) {
          setError(t('dailyLimit.buyUnavailable'));
        } else if (result.status === 401) {
          setError(t('dailyLimit.buyUnauthorized'));
        } else {
          const message = !result.ok ? result.message : undefined;
          setError(message || t('dailyLimit.buyFailed'));
        }
        return;
      }
      ...
      <View style={[styles.sheet, { maxWidth: cardMaxW }]}>
        ...
        <View style={styles.inner}>
          ...
          <View style={styles.emailWrap}>
            <Input
              ...
              baseInputProps={{
                ...
                placeholder: t('dailyLimit.emailPlaceholder'),
                editable: !busy,
              }}
            />
          </View>
          ...
          <Button style={styles.button} onPress={handleBuy} disabled={busy}>
            {busy ? t('dailyLimit.buyLoading') : t('dailyLimit.buyCta')}
          </Button>
          <Button
